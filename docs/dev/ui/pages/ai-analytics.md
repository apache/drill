...
- **Feature labels** identify which UI surface originated the call — see [`PROSPECTOR.md`](../../PROSPECTOR.md#chat-endpoint) for the `context.feature` contract. The label map lives in [`src/constants/aiFeatures.ts`](../../../../exec/java-exec/src/main/resources/webapp/src/constants/aiFeatures.ts) and drives both the feature column and the events feature filter, so a slug missing from it is unselectable in the UI. `aiFeatures.test.ts` scans the source tree for emitted `feature: '…'` literals and fails when this table and the map drift apart. As implemented, the values are:
