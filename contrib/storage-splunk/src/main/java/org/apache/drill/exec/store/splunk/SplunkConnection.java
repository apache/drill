...
public class SplunkConnection {
  ...
  /**
   * As of version 1.8, Splunk's SDK introduced a boolean parameter which is supposed to control whether
   * the SDK will validate SSL certificates or not.  Unfortunately, due to the high quality control standards
   * at Splunk, the parameter does not actually seem to have any effect and the end result is that when making Splunk
   * calls, Splunk will always attempt to verify the SSL certificates, even when the parameter is set to false.
   * This method does what the parameter is supposed to do in the SDK and adds and all trusting SSL Socket Factory to the
   * HTTP client in Splunk's SDK.  In the unlikely event Splunk fixes this issue, we can remove this method.
   * @return A {@link SSLSocketFactory} which trusts any SSL certificate, even ones from Splunk
