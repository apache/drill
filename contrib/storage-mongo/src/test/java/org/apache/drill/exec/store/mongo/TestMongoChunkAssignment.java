...
public class TestMongoChunkAssignment {
  ...
  public void testMongoGroupScanAssignmentMix() throws UnknownHostException, ExecutionSetupException {
    Map<String, Set<ServerAddress>> chunksMapping = Maps.newHashMap();
    Map<String, List<ChunkInfo>> chunksInverseMapping = Maps.newHashMap();
    ...
    MongoGroupScan mongoGroupScan = new MongoGroupScan();
  ...
  public void testMongoGroupScanAssignmentNoAffinity() throws UnknownHostException, ExecutionSetupException {
    ...
    //assignments for chunks on host D, assign on drill bit M
    ...
    //assignments for chunks on host D, assign on drill bit L
    assertEquals(1, mongoGroupScan.getSpecificScan(2).getChunkScanSpecList().size());
    //assignments for chunks on host C, assign on drill bit X
    assertEquals(2, mongoGroupScan.getSpecificScan(3).getChunkScanSpecList().size());
