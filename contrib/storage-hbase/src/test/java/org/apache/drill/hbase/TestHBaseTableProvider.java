...
import org.apache.drill.exec.store.hbase.config.HBaseStoreProvider;
...
public class TestHBaseTableProvider extends BaseHBaseTest {
  ...
  private static HBaseStoreProvider provider;

  @BeforeClass // mask HBase cluster start function
  public static void setUpBeforeTestHBaseTableProvider() throws Exception {
    provider = new HBaseStoreProvider(storagePluginConfig.getHBaseConf(), "drill_store");
    provider.start();
  }
  ...
  public void testTableProvider() throws StoreException {
    ...
    Store<String> hbaseStore = provider.getStore(StoreConfig.newJacksonBuilder(lp.getMapper(), String.class).name("hbase").build());
    hbaseStore.put("", "v0");
    ...
    Store<String> hbaseTestStore = provider.getStore(StoreConfig.newJacksonBuilder(lp.getMapper(), String.class).name("hbase.test").build());
