...
public class SolrClientAPIExec {
  ...
  public CVSchema getSchemaForCore(String coreName) {
    ...
    try {
      ...
      logger.info("schema info... " + oCVSchema.getDefaultSearchField());
    } catch (Exception e) {
  ...
  public SolrDocumentList getSolrDocs(String solrServer,String solrCoreName){
    ...
    SolrQuery solrQuery = new SolrQuery().setTermsRegexFlag("case_insensitive").setRows(100);
  ...
  public List<Tuple> getSolrResponse(String solrServer,SolrClient solrClient, String solrCoreName,
      Map<String,String> solrParams) {
    logger.info("sending request to solr server "+solrServer+" on core "+solrCoreName);
