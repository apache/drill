...
/**
 * The Cassandra storage plugin accepts filters which are:
 * <ul>
 ...
 * </ul>
 */
public class CassandraPushDownListener implements FilterPushDownListener {
  ...
  public boolean isTargetScan(GroupScan groupScan) {
    return groupScan instanceof CassandraGroupScan;
  }

  @Override
  public ScanPushDownListener builderFor(GroupScan groupScan) {
    CassandraGroupScan cassandraGroupScan = (CassandraGroupScan) groupScan;
    if (cassandraGroupScan.hasFilters()) {
      return null;
    } else {
      return new CassandraScanPushDownListener(cassandraGroupScan);
    }
  }

  private static class CassandraScanPushDownListener implements ScanPushDownListener {

    private final CassandraGroupScan groupScan;
    private final Map<String, String> filterParams = CaseInsensitiveMap.newHashMap();

    CassandraScanPushDownListener(CassandraGroupScan groupScan) {
      this.groupScan = groupScan;
      for (SchemaPath field : groupScan.getColumns()) {
    ...
    public Pair<GroupScan, List<RexNode>> transform(AndNode andNode) {
      ...
      CassandraGroupScan newScan = new CassandraGroupScan(groupScan, filters, selectivity);
