...
public class RangerBaseAuthorizer {
  private static final Logger logger = LoggerFactory.getLogger(RangerBaseAuthorizer.class);
  ...
  public synchronized void init(String serviceName) {
    ...
    try {
      ...
      plugin.init();
      logger.info("RangerPlugin initialized successfully for serviceName: {}", serviceName);
    } catch (Exception e) {
      plugin = null;
