...
public class DrillAuthorizer {
   ...
   * Controls the depth of validation in the {@link #validateResource(DrillResource, ValidationLevel)}
  ...
  public boolean checkTableAccess(DrillResource resource, DrillAccessType operator) {
    if (!validateResource(resource, ValidationLevel.TABLE)) {
      logger.warn("MetaStoreResource validation failed for table access check");
      return false;
    }
