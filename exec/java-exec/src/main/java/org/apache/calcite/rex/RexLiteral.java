...
public class RexLiteral extends RexNode {
  ...
  RexLiteral(
    ...
    // DRILL PATCH: Skip validation for ANY type to work around Calcite 1.38 regression (CALCITE-6427)
    // where type inference creates Sargs with ANY types that are only used during
    // toString/digest operations, not actual query execution
    if (typeName != SqlTypeName.ANY) {
      checkArgument(valueMatchesType(value, typeName, true));
      checkArgument((value == null) == type.isNullable());
    }
    // checkArgument(typeName != SqlTypeName.ANY);  // Disabled to allow ANY type
  ...
  private static void appendAsJava(@Nullable Comparable value, StringBuilder sb,
    ...
    case GEOMETRY:
      ...
      break;
    case ANY:
      // DRILL PATCH: Handle ANY type for Calcite 1.38 regression (CALCITE-6427)
      // ANY types appear in Sargs during type inference bugs, only used for digest/toString
      // Skip the assert since valueMatchesType returns false for ANY
      sb.append(value != null ? value.toString() : "null");
      break;
    default:
      assert valueMatchesType(value, typeName, true) : "value " + value + " does not match type " + typeName;
   ...
   * also literals.
   *
   * DRILL PATCH: Handle ANY type which can occur in Calcite 1.38 due to type inference bugs.
   * RexLiteral explicitly forbids ANY type, so we infer a reasonable type from the value.
   */
  private static RexLiteral toLiteral(RelDataType type, Comparable<?> value) {
    final SqlTypeName typeName = strictTypeName(type);

    // DRILL PATCH: No special handling needed here - we've disabled the ANY check in the constructor
