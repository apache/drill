...
public class DateTypeFunctions {
  ...
  public static class AgeTimeStampFunction implements DrillSimpleFunc {
    ...
    @Override
    public void setup() {
      zoneId = contextInfo.getRootFragmentTimeZone();
    }

    @Override
    public void eval() {
      java.time.Instant fromInst = org.apache.drill.exec.vector.DateUtilities.fromDrillTimestamp(right.value, zoneId);
      java.time.ZonedDateTime from = fromInst.atZone(zoneId);
      java.time.Instant toInst = org.apache.drill.exec.vector.DateUtilities.fromDrillTimestamp(left.value, zoneId);
      java.time.ZonedDateTime to = toInst.atZone(zoneId);
  ...
  @FunctionTemplate(name = "age", scope = FunctionTemplate.FunctionScope.SIMPLE, nulls = NullHandling.NULL_IF_NULL)
  public static class AgeTimeStampUnaryFunction implements DrillSimpleFunc {
    @Param
    TimeStampHolder right;
    @Workspace
    ...
    public void setup() {
      zoneId = contextInfo.getRootFragmentTimeZone();
      java.time.ZonedDateTime queryStart = contextInfo.getQueryStartInstant().atZone(zoneId);
      to = queryStart.truncatedTo(java.time.temporal.ChronoUnit.DAYS);
    }
  ...
  @FunctionTemplate(name = "age", scope = FunctionTemplate.FunctionScope.SIMPLE, nulls = NullHandling.NULL_IF_NULL)
  public static class AgeDateUnaryFunction implements DrillSimpleFunc {
    @Param
    DateHolder right;
    @Workspace
    java.time.ZonedDateTime to;
