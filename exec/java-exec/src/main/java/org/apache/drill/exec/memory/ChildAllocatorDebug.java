...
/**
 * Child allocator class. Only slightly different from the {@link RootAllocator},
 * in that these can't be created directly, but must be obtained from
 * {@link BufferAllocator#newChildAllocator(AllocatorOwner, long, long, int)}.
...
class ChildAllocatorDebug extends BaseAllocatorDebug {
   ...
   *   note this includes direct allocations (via {@link BufferAllocator#buffer(int, int)}
