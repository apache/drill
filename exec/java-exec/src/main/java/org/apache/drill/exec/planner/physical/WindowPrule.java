...
public class WindowPrule extends Prule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    for (final Ord<Window.Group> w : Ord.zip(window.groups)) {
      ...
      List<RelDataTypeField> newRowFields = new ArrayList<>(convertedInput.getRowType().getFieldList());

      Iterable<RelDataTypeField> newWindowFields = window.getRowType().getFieldList().stream()
          .filter(relDataTypeField -> relDataTypeField.getName().startsWith("w" + w.i + "$"))
      ...
      List<Window.RexWinAggCall> newWinAggCalls = new ArrayList<>();
      for (Ord<Window.RexWinAggCall> aggOrd : Ord.zip(windowBase.aggCalls)) {
        ...
        List<RexNode> newOperandsOfWindowFunction = new ArrayList<>();
