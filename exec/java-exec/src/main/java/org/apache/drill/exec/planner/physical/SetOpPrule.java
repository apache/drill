...
import org.apache.drill.exec.planner.logical.DrillSetOpRel;
...
public class SetOpPrule extends Prule {
  public static final RelOptRule INSTANCE = new SetOpPrule();
  protected static final Logger tracer = CalciteTrace.getPlannerTracer();

  private SetOpPrule() {
    super(RelOptHelper.any(DrillSetOpRel.class), "Prel.SetOpPrule");
  }

  @Override
  public void onMatch(RelOptRuleCall call) {
    final DrillSetOpRel setOpRel = call.rel(0);
    final List<RelNode> inputs = setOpRel.getInputs();
    ...
    for (RelNode child : inputs) {
      ...
      if (settings.isUnionAllDistributeEnabled()) {
        /*
         * Strictly speaking, set op does not need re-distribution of data; but in Drill's execution
    ...
    try {
      ...
      if (allHashDistributed) {
        // since all children of set op are hash distributed, propagate the traits of the left child
        traits = convertedInputList.get(0).getTraitSet();
      } else {
        // output distribution trait is set to ANY since set-op inputs may be distributed in different ways
        ...
        // inserting an Exchange after the set-op.
        traits = call.getPlanner().emptyTraitSet().plus(Prel.DRILL_PHYSICAL).plus(DrillDistributionTrait.ANY);
      }

      Preconditions.checkArgument(convertedInputList.size() == 2, "inputs of set op must be two items.");
      RelNode left = convertedInputList.get(0);
      for (int i = 1; i < convertedInputList.size(); i++) {
        left = new SetOpPrel(setOpRel.getCluster(), traits, ImmutableList.of(left, convertedInputList.get(i)), setOpRel.kind, setOpRel.all);
      }
