...
@Output ${type.to}Holder out;
    ...
    public void eval() {
        ...
        int carry = (org.apache.arrow.vector.util.DecimalUtility.getFirstFractionalDigit(in.value, in.scale) > 4) ? 1 : 0;
        // Assign the integer part of the decimal to the output holder
        out.value = java.lang.Math.abs((${type.javatype}) (org.apache.arrow.vector.util.DecimalUtility.adjustScaleDivide(in.value, (int) in.scale))) + carry;
        if (in.value < 0) {
          out.value *= -1;
        }
    }
...
@Output ${type.to}Holder out;
    ...
    public void eval() {
        ...
        int carry = (org.apache.arrow.vector.util.DecimalUtility.getFirstFractionalDigit(in.buffer, in.scale, in.start, in.nDecimalDigits) > 4) ? 1 : 0;

        // Get the index, where the integer part of the decimal ends
        int integerEndIndex = in.nDecimalDigits - org.apache.arrow.vector.util.DecimalUtility.roundUp(in.scale);
        ...
        for (int i = 0 ; i < integerEndIndex; i++) {
            ...
            out.value = (${type.javatype}) ((out.value * org.apache.arrow.vector.util.DecimalUtility.DIGITS_BASE) + in.getInteger(i, in.start, in.buffer));
        }
