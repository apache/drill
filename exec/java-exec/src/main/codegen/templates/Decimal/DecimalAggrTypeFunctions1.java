...
public static class ${type.inputType}${aggrtype.className} implements DrillAggFunc{
  ...
  public void setup() {
    ...
    org.apache.arrow.vector.util.Text tmp = new org.apache.arrow.vector.util.Text(byteArray);
    value.obj = tmp;
    <#if aggrtype.funcName == "max">
    for (int i = 0; i < ${type.runningType}Holder.nDecimalDigits; i++) {
      org.apache.arrow.vector.util.ByteFunctionHelpers.setInteger(tmp.getBytes(), i, 0xFFFFFFFF);
    }
    org.apache.arrow.vector.util.ByteFunctionHelpers.setSign(tmp.getBytes(), true);
    <#elseif aggrtype.funcName == "min">
    for (int i = 0; i < ${type.runningType}Holder.nDecimalDigits; i++) {
      org.apache.arrow.vector.util.ByteFunctionHelpers.setInteger(tmp.getBytes(), i, 0x7FFFFFFF);
    }
    // Set sign to be positive so initial value is maximum
    org.apache.arrow.vector.util.ByteFunctionHelpers.setSign(tmp.getBytes(), false);
    </#if>
  ...
  public void add() {
    ...
    <#if type.outputType.endsWith("Sparse")>
      ...
      org.apache.arrow.vector.util.Text tmp = (org.apache.arrow.vector.util.Text) value.obj;
      int cmp = org.apache.arrow.vector.util.DecimalUtility.compareSparseSamePrecScale(in.buffer, in.start, tmp.getBytes(), tmp.getLength());
    if (cmp == 1) {
      //in.buffer.getBytes(in.start, tmp.getBytes(), 0, ${type.runningType}Holder.WIDTH);
      for (int i = 0; i < ${type.runningType}Holder.nDecimalDigits; i++) {
        org.apache.arrow.vector.util.ByteFunctionHelpers.setInteger(tmp.getBytes(), i, ${type.runningType}Holder.getInteger(i, in.start, in.buffer));
      }
      org.apache.arrow.vector.util.ByteFunctionHelpers.setSign(tmp.getBytes(), in.getSign(in.start, in.buffer));
  ...
  public void output() {
    ...
    org.apache.arrow.vector.util.DecimalUtility.getSparseFromBigDecimal((java.math.BigDecimal) value.obj, out.buffer, out.start, out.scale, out.precision, out.nDecimalDigits);
   <#else>
    <#if type.outputType.endsWith("Sparse")>
    org.apache.arrow.vector.util.Text tmp = (org.apache.arrow.vector.util.Text) value.obj;
    ...
    for (int i = 0; i < ${type.runningType}Holder.nDecimalDigits; i++) {
      ${type.runningType}Holder.setInteger(i, org.apache.arrow.vector.util.ByteFunctionHelpers.getInteger(tmp.getBytes(), i), 0, buffer);
    }
    out.buffer = buffer;
    out.start = 0;
    out.setSign(org.apache.arrow.vector.util.ByteFunctionHelpers.getSign(tmp.getBytes()), out.start, out.buffer);
