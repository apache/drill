...
public class TestUserSessionInStorage extends ClusterTest {
  ...
  public void testAnonymousUserSession() throws Exception {
    ...
    StoragePlugin plugin = registry.getPlugin("dfs");

    // Get the user session.  This should not be null.
    assertNotNull(plugin.getUserSession());
    assertEquals("anonymous", plugin.getActiveUsername());
  }
  ...
  public void testWithUser() throws Exception {
    ...
    // Get the user session.  This should not be null.
    assertNotNull(plugin.getUserSession());
    assertEquals(TEST_USER_1, plugin.getActiveUsername());
  }
  ...
  public void testUserUpdate() throws Exception {
    ...
    client.queryBuilder().sql(sql).run();
    assertEquals(TEST_USER_2, plugin.getActiveUsername());
  }
}
