 ...
 * before 1.6.0. The values have been read by Drill correctly by Drill, but
...
public class TestCorruptParquetDateCorrection extends PlanTestBase {
  ...
  //    - one created with the fixed version of the reader, right before 1.6
  //        - the code was changed to write the version number 1.6 (without snapshot) into the file
  //        - for compatibility all 1.6-SNAPSHOT files are read to correct the corrupt dates
  ...
  private static final String CORRECTED_PARTITIONED_DATES_1_6_PATH =
      "[WORKING_PATH]/src/test/resources/parquet/4203_corrupt_dates/1_6_0_partitioned_no_corruption";
  ...
  public static void initFs() throws Exception {
    ...
    copyDirectoryIntoTempSpace(CORRECTED_PARTITIONED_DATES_1_6_PATH, MIXED_CORRUPTED_AND_CORRECTED_PARTITIONED_FOLDER);
   ...
   * number of 1.6.0 in their footers, so we can be certain they do not have corruption. The option to disable the
    ...
    try {
      for (String selection : new String[]{"*", "date_col"}) {
        ...
        TestBuilder builder = testBuilder()
            .sqlQuery("select " + selection + " from table(dfs.`" + CORRECTED_PARTITIONED_DATES_1_6_PATH + "`" +
        ...
        String query = "select " + selection + " from table(dfs.`" + CORRECTED_PARTITIONED_DATES_1_6_PATH + "` " +
