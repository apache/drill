...
/*
  ...
  public void testHashTable1(@Injectable final DrillbitContext bitContext, @Injectable UserClientConnection connection) throws Throwable {
    final String plan_path = "/common/test_hashtable1.json";
    final SimpleRootExec exec = doTest(bitContext, connection, plan_path);

    final SchemaPath regionkey = new SchemaPath("regionkey", ExpressionPosition.UNKNOWN);
    final SchemaPath nationkey = new SchemaPath("nationkey", ExpressionPosition.UNKNOWN);

    final NamedExpression[] keyExprs = new NamedExpression[1];
    keyExprs[0] = new NamedExpression(regionkey, new FieldReference(regionkey));

    final HashTableConfig htConfig = new HashTableConfig(HashTable.DEFAULT_INITIAL_CAPACITY, HashTable.DEFAULT_LOAD_FACTOR, keyExprs);
    final ChainedHashTable cht = new ChainedHashTable(htConfig, exec.getContext(), exec.getIncoming(), null);
    final HashTable htable = cht.createAndSetupHashTable();
    final IntHolder htIdxHolder = new IntHolder();
