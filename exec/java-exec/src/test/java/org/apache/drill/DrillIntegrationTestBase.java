...
public abstract class DrillIntegrationTestBase extends DrillTestBase {
  ...
  public static TestBuilder testBuilder() {
    assert cluster != null;
    return new TestBuilder(cluster.allocator());
  }
  ...
  public static void beforeClass() throws Exception {
    ...
    Scope scope = getCurrentClusterScope(getTestClass());

    switch (scope) {
      case GLOBAL:
      case SUITE:
        cluster = new DrillTestCluster(size, width, config);
        break;
      default:
  ...
  protected static final class DrillTestCluster implements Closeable {
    ...
    public DrillClient getNewClient(Properties properties) throws RpcException {
      return QueryTestUtil.createClient(config, serviceSet, width, properties);
    }
    ...
    public String tmpSchemaLocation() {
      return tmpSchemaLocation;
    }
   ...
   *
   * @return DrillbitContext of a random Drillbit in the cluster.
   */
  protected static DrillbitContext getDrillbitContext() {
    Preconditions.checkState(cluster != null, "Test cluster is not running.");
    return cluster.randomDrillBitContext();
  }
  ...
  protected static void updateTestCluster(int newDrillbitCount, DrillConfig newConfig) {
    fail("Please use @ClusterScope annotations to set cluster properties");
  }
