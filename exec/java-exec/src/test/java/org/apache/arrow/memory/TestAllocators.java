...
public class TestAllocators extends DrillTest {
  ...
  private static final Properties TEST_CONFIGURATIONS = new Properties() {
    {
      put(org.apache.arrow.memory.RootAllocatorFactory.TOP_LEVEL_MAX_ALLOC, "14000000");
  ...
  /**
   * Contract for ArrowBuf[] returned from getBuffers() is that buffers are returned in a reader appropriate state
   * (i.e., readIndex = 0)
   *
   * Before this patch, the following scenario breaks this contract:
   * As data being read from ArrowBuf, readIndex will be pushed forward. And, later on,
   * when ArrowBuf[] are read from the ValueVector, readIndex will point at the location of the most recent reading
   *
   * This unit test is added to ensure that the readIndex points at zero under this scenario
   */
  @Test // DRILL-3854
  public void ensureArrowBufReadIndexIsZero() throws Exception {
    final int length = 10;

    final Properties props = new Properties() {
      {
        put(org.apache.arrow.memory.RootAllocatorFactory.TOP_LEVEL_MAX_ALLOC, "1000000");
      }
    };
    final DrillConfig config = DrillConfig.create(props);

    final BufferAllocator allc = org.apache.arrow.memory.RootAllocatorFactory.newRoot(config);
    ...
    // Write data to ArrowBuf
    for(int i = 0; i < length; ++i) {
      iv.getBuffer().writeInt(i);
    }

    // Read data to ArrowBuf
    for(int i = 0; i < length; ++i) {
      assertEquals(i, iv.getBuffer().readInt());
    }

    for(ArrowBuf ArrowBuf : iv.getBuffers(false)) {
      assertEquals(0, ArrowBuf.readInt());
    }

    final List<ArrowBuf> toBeClean = Lists.newArrayList();
    for(ArrowBuf ArrowBuf : iv.getBuffers(true)) {
      assertEquals(0, ArrowBuf.readInt());
      toBeClean.add(ArrowBuf);
    }

    for(ArrowBuf ArrowBuf : toBeClean) {
      ArrowBuf.release();
    }
  ...
  public void testTransfer() throws Exception {
    final Properties props = new Properties() {
      {
        put(org.apache.arrow.memory.RootAllocatorFactory.TOP_LEVEL_MAX_ALLOC, "1049600");
      }
    };
    final DrillConfig config = DrillConfig.create(props);
    BufferAllocator a = org.apache.arrow.memory.RootAllocatorFactory.newRoot(config);
    ...
    ArrowBuf buf1 = a1.buffer(1_000_000);
    ArrowBuf buf2 = a2.buffer(1_000);
    ArrowBuf buf3 = buf1.transferOwnership(a2).buffer;
  ...
  public void testAllocators() throws Exception {
    ...
    try (final RemoteServiceSet serviceSet = RemoteServiceSet.getLocalServiceSet();
      ...
      ArrowBuf b11 = oContext11.getAllocator().buffer(1000000);

      OperatorContext oContext12 = fragmentContext1.newOperatorContext(physicalOperator2, stats);
      ArrowBuf b12 = oContext12.getAllocator().buffer(500000);
      ...
      ArrowBuf b22 = oContext22.getAllocator().buffer(2000000);
      ...
      ArrowBuf b31a = oContext31.getAllocator().buffer(200000);
      ...
      try {
        ArrowBuf b32 = oContext32.getAllocator().buffer(4400000);
        b32.release();
      } catch (OutOfMemoryException e) {
