...
public class RunTimeScan {
  ...
  static Collection<URL> getNonPrescannedMarkedPaths() {
    Collection<URL> markedPaths = ClassPathScanner.getMarkedPaths(CommonConstants.DRILL_JAR_MARKER_FILE_RESOURCE_PATHNAME);
  ...
  public static ScanResult fromPrescan(DrillConfig config) {
    ...
    if (ClassPathScanner.isScanBuildTimeCacheEnabled(config)) {
      ...
      return ClassPathScanner.scan(
          ClassPathScanner.getMarkedPaths(CommonConstants.DRILL_JAR_MARKER_FILE_RESOURCE_PATHNAME),
          packagePrefixes,
          scannedBaseClasses,
          scannedAnnotations,
